...
const endpoints=["functions/api/geocode.ts","functions/api/timezone.ts","functions/api/osm-site-context.ts","functions/api/gsi-geoid.ts"];
...
// 2026-08-28追記: gsi-elevation.ts（地形取得API）は、以前「複数座標を
// まとめた外側のバッチキャッシュ」（getOrCreateR2Json経由）を使って
// いたが、三脚探索の性質上ほとんど意味をなさなかったため撤去した。
// 代わりに、本当に効果のある「DEMタイル単位のR2永続キャッシュ」
// （lookupGsiElevations→server/gsiElevation.ts→r2PersistentCache.ts経由、
// 内部でallowR2Read/reserveR2Writeを使う）を直接呼ぶ設計になっている。
// 安全ガードを経由していることは変わらない（経路が変わっただけ）ため、
// それを別途確認する。
{
  const gsiElevationEndpoint = fs.readFileSync(path.join(root,"functions/api/gsi-elevation.ts"),"utf8");
  const gsiElevationServer = fs.readFileSync(path.join(root,"server/gsiElevation.ts"),"utf8");
  const r2PersistentCache = fs.readFileSync(path.join(root,"server/r2PersistentCache.ts"),"utf8");
  check(
    "functions/api/gsi-elevation.ts uses tile-level R2 safety guard",
    gsiElevationEndpoint.includes("lookupGsiElevations(") &&
    gsiElevationServer.includes("serverPersistentCache(") &&
    r2PersistentCache.includes("allowR2Read") &&
    r2PersistentCache.includes("reserveR2Write")
  );
}
