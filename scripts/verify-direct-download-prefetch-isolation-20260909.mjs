...
const checks = [
  ...
  ['bulk terrain pass resumes before final tile drain', mgr.includes('resumeDeviceTilePrefetch();') && mgr.includes('flushGsiDeviceTilePrefetchQueue(signal)')],
  ['tile drain occurs after terrain workers finish', mgr.indexOf('await Promise.all(Array.from({ length: workerCount }, () => worker()));') < mgr.indexOf('await flushGsiDeviceTilePrefetchQueue(signal)')],
  ['foreground precision unchanged at 1m', mgr.includes('sampleWorldTerrainNeutral(terrainPoints, signal, "1m",') && mgr.includes('allowWorldTerrainFallback: false')],
