...
const checks = [
  ...
  ['existing tripod direction observer uses current UI camera height via shared helper', /withLensCenterHeight\(\s*tripodPointRef\.current,\s*cameraSettings\.lensCenterHeightMeters/s.test(app)],
  ...
  ['final candidate observer uses same current camera height', /withLensCenterHeight\(\s*finalConvergencePoint,\s*lensCenterHeightMeters/s.test(tripod)],
