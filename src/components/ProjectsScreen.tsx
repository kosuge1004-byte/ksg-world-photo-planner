...
export function ProjectsScreen({ open, projects, onBack, onLoad, onUpdate, onDelete, onShare, onImport }: Props) {
  ...
  if (!open) return null;
  return <section className="project-screen"><header><button type="button" className="project-screen-back" onClick={onBack} aria-label="メイン画面へ戻る">‹ 戻る</button><h1>プロジェクト</h1><span /></header>
    ...
    : <div className="project-list">{projects.length===0&&<p>保存済みプロジェクトはありません。</p>}{projects.map(p=><article key={p.id}><button className="project-main" onClick={()=>onLoad(p)}><strong>{p.name}</strong><span>{p.shootingDateTimeLocal.replace("T"," ")}</span></button><div className="project-row-options"><label className="check"><input type="checkbox" checked={p.calendarRegistered} onChange={e=>onUpdate({...p,calendarRegistered:e.target.checked,updatedAtIso:new Date().toISOString()})}/>カレンダーへ登録</label><button type="button" onClick={()=>onShare(p)}>共有</button></div><div><button onClick={()=>setEditing(p)}>編集</button><button className="danger" onClick={()=>{if(confirm("このプロジェクトを削除しますか？"))onDelete(p.id)}}>削除</button></div></article>)}</div>}
  </section>;
}
