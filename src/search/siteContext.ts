...
export async function fetchSiteContexts(
  ...
  // water-only はサーバー側で最大3,000地点を1回の軽量Overpass問い合わせへ
  // 集約できる。河川の最近傍陸地探索（10半径×8方向）を10回直列通信に
  // しないため、ここでは1リクエストで送る。
  if (purpose === "water-only") {
    fetched = [];
    for (let offset = 0; offset < missingPoints.length; offset += 3_000) {
      fetched.push(...await fetchWaterContextBatchResilient(missingPoints.slice(offset, offset + 3_000), signal));
    }
  } else {
    fetched = [];
